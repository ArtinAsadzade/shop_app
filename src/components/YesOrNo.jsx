...
export default function YesOrNo({ title, desc, func, button, show, setShow }) {
  ...
  return createPortal(
    ...
    >
      <div className={`w-full max-w-sm p-7 z-40 text-gray-500 bg-white rounded-lg shadow-xl border-2`} role="alert">
        <div className="flex">
          <div className="ms-3 text-sm w-full font-normal">
            <span className="mb-1 text-lg font-semibold text-gray-900">{title}</span>
            <div className="mb-2 text-sm font-normal">{desc}</div>
            {button ? (
              <div className="grid grid-cols-2 w-full gap-2">
                <div>
                  <button
                    className="inline-flex justify-center w-full px-2 py-1.5 text-xs font-medium text-center text-white bg-primary rounded-lg hover:bg-red-200 focus:ring-4 focus:outline-none focus:ring-gray-700"
                    onClick={handleClick}
                  >
                    Yes
                  </button>
